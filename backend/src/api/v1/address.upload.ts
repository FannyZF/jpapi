...
router.post("/jp/cleanse/address/upload", upload.single("file"), async (req: Request, res: Response) => {
  try {
    ...
    const trackingCol = (req.body.tracking_col as string) || findColumn(headers, COLUMN_ALIASES.tracking);
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const tracking = trackingIdx >= 0 ? String(row[trackingIdx] || "") : "";
      const refId = crypto.randomUUID();

      if (!addr.trim() || !zip.trim()) {
        rows.push({ refId, pref, city, ward, addr, zip, tracking, status: "error", message: "Missing address/zipcode", dashSegments: [], fullAddr: "", validatedBase: "", validatedPref: "", validatedCity: "", validatedWard: "", validatedStreet: "", validatedZip: zip, validatedFull: "", validationLevel: "", correction: "" });
        continue;
      }
      ...
      try {
        ...
        if (address.is_valid) {
          ...
          rows.push({
            refId, pref, city, ward, addr, zip, tracking,
            status: "verified",
        ...
        rows.push({ refId, pref, city, ward, addr, zip, tracking, status: "error", message: "Processing error", dashSegments: [], fullAddr, validatedBase: "", validatedPref: "", validatedCity: "", validatedWard: "", validatedStreet: "", validatedZip: zip, validatedFull: "", validationLevel: "", correction: "" });
      }
    }
